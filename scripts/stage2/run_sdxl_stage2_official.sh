...
# Usage:
#   bash scripts/stage2/run_sdxl_stage2_official.sh <dataset_root> <stage1_render_records_jsonl> [batch_size] [epochs] [extra args...]
# Environment helpers:
...
# Example:
#   STAGE2_NUM_PROCESSES=2 bash scripts/stage2/run_sdxl_stage2_official.sh /data/imagenette/train runs/stage1/render/imagenette/qwen_local/2026-04-02_010203/records.jsonl 1 1 --learning-rate 1e-4

if [[ $# -lt 2 ]]; then
  echo "Usage: bash scripts/stage2/run_sdxl_stage2_official.sh <dataset_root> <stage1_render_records_jsonl> [batch_size] [epochs] [extra args...]"
  exit 1
fi
...
bash scripts/stage2/check_stage2_sdxl_env.sh "${CSPD_STAGE2_SDXL_SCRIPT:-}" >/dev/null
