...
#   bash scripts/stage1/run_stage1_qwen_local.sh /data/imagenette 256 DEFAULT 10 /data/imagenette/class_to_archetype.json
#
...
if [[ $# -lt 1 ]]; then
  echo "Usage: bash scripts/stage1/run_stage1_qwen_local.sh <dataset_root> [max_new_tokens] [class_name_map] [flush_every] [class_archetype_map]"
  exit 1
fi
