...
# Usage:
#   bash scripts/prep/prepare_stage1_metadata.sh <classes_py_or_json> <class_archetype_json> [class_var_name]

if [[ $# -lt 2 ]]; then
  echo "Usage: bash scripts/prep/prepare_stage1_metadata.sh <classes_py_or_json> <class_archetype_json> [class_var_name]"
  exit 1
fi
...
if [[ "$CLASSES_SOURCE" == *.json ]]; then
  ...
  CMD=(python scripts/prep/convert_class_py_to_json.py --input "$CLASSES_SOURCE" --output "$CLASSES_JSON")
