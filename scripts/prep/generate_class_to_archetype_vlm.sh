...
# Usage:
#   bash scripts/prep/generate_class_to_archetype_vlm.sh <dataset_root> [images_per_class] [taxonomy_json]
#
# By default this script uses the repo-bundled classes.json at the repo root.

if [[ $# -lt 1 ]]; then
  echo "Usage: bash scripts/prep/generate_class_to_archetype_vlm.sh <dataset_root> [images_per_class] [taxonomy_json]"
  exit 1
fi
