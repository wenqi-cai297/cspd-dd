...
#
# Usage:
#   bash scripts/server/stage2/run_stage2_pipeline.sh <dataset_root> <stage1_render_records_jsonl> [batch_size] [epochs] [rank]
#
# Examples:
#   # ImageNette with default config (best known: rank=64, epoch=15, batch=8)
#   bash scripts/server/stage2/run_stage2_pipeline.sh \
...
if [[ $# -lt 2 ]]; then
  echo "Usage: bash scripts/server/stage2/run_stage2_pipeline.sh <dataset_root> <stage1_render_records_jsonl> [batch_size] [epochs] [rank]"
  exit 1
fi
